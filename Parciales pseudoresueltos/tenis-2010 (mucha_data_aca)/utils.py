...
def trackbar_sobel(imagen):
    def transformacion(imagen, valores_trackbar):
        ...
        if dx == 1:
            sobel_x = cv.Sobel(imagen, ddepth, dx, 0, ksize)
            sobel_x = cv.convertScaleAbs(sobel_x)
            salida += sobel_x
        if dy == 1:
            sobel_y = cv.Sobel(imagen, ddepth, 0, dy, ksize)
...
def dibujar_intersecciones(imagen, intersecciones):
    ...
    cv.imshow("Imagen con intersecciones", imagen)
    cv.waitKey(0)
