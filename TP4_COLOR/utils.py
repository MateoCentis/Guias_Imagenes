...
def ventana_trackbars(imagen, variables_trackbar = None, parametros_trackbar = None, transformacion = None):
  ...
  while True:
    ...
    #otra alternativa, con la imagen original flotante
    # cv.imshow("Imagen original",imagen)
    # cv.imshow(NOMBRE_VENTANA,imagen_salida)

    #Alternativa con subplots (se ve mejor pero las trackbar quedan en otra ventana y tenes que cerrar el gráfico para act)
     # Crear subplots
