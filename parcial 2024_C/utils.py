...
########################################################################################
def trackbar_filtro_fastNlMeans(imagen):
...
def trackbar_filtro_mediana(imagen):
...
def segmentar_RGB(imagen_BGR,R,G,B,radio):
    ...
    mascara = np.uint8(mascara_rgb * 255)  # Convertimos la máscara a tipo uint8


    imagen_bgr_segmentada = cv.bitwise_and(imagen_BGR, imagen_BGR, mask=mascara_rgb)    
    
    return imagen_bgr_segmentada, mascara
...
def trackbar_hough_lineas_and_canny(imagen):
    ...
    parametros_trackbar = [[0, 255],[0, 1000], [0,360],[0, 360]]
...
# Aplicar cv2.connectedComponentsWithStats
# num_labels, labels, stats, centroids = cv.connectedComponentsWithStats(imagen_binaria_invertida, connectivity=8, ltype=cv.CV_32S)


# Extraer las áreas de los componentes (omitir el primer componente que es el fondo)
    # areas = stats[1:, cv.CC_STAT_AREA]

    # # Crear un diccionario con las estadísticas de los componentes
    # componentes_estadisticas = {
    #     "num_labels": num_labels - 1,  # Restar 1 para no contar el fondo
    #     "areas": areas,
    #     "stats": stats[1:],  # Omitir la primera fila que corresponde al fondo
    #     "centroids": centroids[1:]  # Omitir el primer centroid que corresponde al fondo
    # }

def encontrar_componentes_y_posiciones(imagen_binaria):
    # Verificar que la imagen es binaria
    if len(np.unique(imagen_binaria)) > 2:
        raise ValueError("La imagen no es binaria. Asegúrate de que solo contenga valores 0 y 255.")
    
    # Aplicar cv2.connectedComponentsWithStats
    num_labels, labels, stats, centroids = cv.connectedComponentsWithStats(imagen_binaria, connectivity=8, ltype=cv.CV_32S)
    
    # stats es una matriz donde cada fila corresponde a un componente, y las columnas son:
    # [0] x - La coordenada x del rectángulo delimitador del componente
    # [1] y - La coordenada y del rectángulo delimitador del componente
    # [2] width - El ancho del rectángulo delimitador del componente
    # [3] height - La altura del rectángulo delimitador del componente
    # [4] area - El área del componente
    
    # Extraer las posiciones y áreas de los componentes (omitir el primer componente que es el fondo)
    posiciones = centroids[1:]
    areas = stats[1:, cv.CC_STAT_AREA]

    # Crear una lista de diccionarios con la información de cada círculo
    circulos_info = []
    for i in range(len(areas)):
        circulo_info = {
            "posicion": (posiciones[i][0], posiciones[i][1]),
            "area": areas[i]
        }
        circulos_info.append(circulo_info)

    # Crear un diccionario con la información total de los círculos
    resultado = {
        "num_componentes": num_labels - 1,  # Restar 1 para no contar el fondo
        "componentes": circulos_info
    }

    return resultado

# Ejemplo de uso: 
# imagen_binaria = cv.imread('ruta_a_la_imagen_binaria', cv.IMREAD_GRAYSCALE)
# circulos_info = encontrar_circulos(imagen_binaria)
# print("Número de círculos:", circulos_info["num_circulos"])
# print("Posiciones de los círculos:", circulos_info["posiciones"])
...
# # Cargar la imagen grande y la plantilla
# imagen_grande = cv2.imread('imagen_grande.jpg', 0)
# template = cv2.imread('template.jpg', 0)

# # Obtener las dimensiones de la plantilla
# h, w = template.shape

# # Realizar la coincidencia de plantillas utilizando el método cv2.TM_CCOEFF_NORMED
# result = cv2.matchTemplate(imagen_grande, template, cv2.TM_CCOEFF_NORMED)

# # Encontrar la ubicación del valor máximo en el resultado
# min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)

# # Extraer la posición superior izquierda de la mejor coincidencia
# top_left = max_loc

# # Dibujar un rectángulo alrededor de la coincidencia en la imagen original
# bottom_right = (top_left[0] + w, top_left[1] + h)
# cv2.rectangle(imagen_grande, top_left, bottom_right, 255, 2)

# # Mostrar la imagen con el rectángulo dibujado
# cv2.imshow('Resultado', imagen_grande)
# cv2.waitKey(0)
# cv2.destroyAllWindows()


# cv2.TM_SQDIFF: Este método calcula la suma de las diferencias al cuadrado entre la plantilla y la imagen. El valor más bajo indica la mejor coincidencia.
# cv2.TM_SQDIFF_NORMED: Versión normalizada de cv2.TM_SQDIFF.
# cv2.TM_CCORR: Este método calcula la correlación cruzada entre la plantilla y la imagen. El valor más alto indica la mejor coincidencia.
# cv2.TM_CCORR_NORMED: Versión normalizada de cv2.TM_CCORR.
# cv2.TM_CCOEFF: Este método calcula el coeficiente de correlación entre la plantilla y la imagen. El valor más alto indica la mejor coincidencia.
# cv2.TM_CCOEFF_NORMED: Versión normalizada de cv2.TM_CCOEFF.